...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_failed) {
      tunit::assert::is_not_null(nullptr);
    }
  };
}

void test_(test_assert_is_not_null_nullptr_failed, test_output) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_is_not_null_nullptr_failed.*");
  std::stringstream ss;
...
void test_(test_assert_is_not_null_nullptr_failed, test_result) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_is_not_null_nullptr_failed.*");
  std::stringstream ss;
