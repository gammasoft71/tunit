...
namespace unit_tests {
  class test_class_(test_assert_is_NaN_float_failed) {
  public:
    void test_method_(test_case_failed) {
      float f = 3.14159f;
      tunit::assert::is_NaN(f);
    }
  };
}

void test_(test_assert_is_NaN_float_failed, test_output) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_is_NaN_float_failed.*");
  std::stringstream ss;
...
void test_(test_assert_is_NaN_float_failed, test_result) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_is_NaN_float_failed.*");
  std::stringstream ss;
