...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      int i = 42;
      tunit::assert::is_greater_or_equal(i, 24);
      tunit::assert::is_greater_or_equal(i, 42);
    }
  };
}
...
void test_(test_assert_is_greater_or_equal_succeed, test_output) {
  ...
  tunit::settings::default_settings().filter_tests("test_assert_is_greater_or_equal_succeed.*");
  std::stringstream ss;
