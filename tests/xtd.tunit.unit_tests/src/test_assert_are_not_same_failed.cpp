...
namespace unit_tests {
  class test_class_(test_assert_are_not_same_failed) {
  public:
...
void test_(test_assert_are_not_same_failed, test_output) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_are_not_same_failed.*");
  std::stringstream ss;
...
void test_(test_assert_are_not_same_failed, test_result) {
  tunit::settings::default_settings().exit_status(0);
  tunit::settings::default_settings().filter_tests("test_assert_are_not_same_failed.*");
  std::stringstream ss;
