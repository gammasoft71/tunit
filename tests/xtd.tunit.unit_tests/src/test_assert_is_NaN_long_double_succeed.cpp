...
void test_(test_assert_is_NaN_long_double_succeed, test_output) {
  ...
  tunit::settings::default_settings().filter_tests("test_assert_is_NaN_long_double_succeed.*");
  std::stringstream ss;
  tunit::unit_test(std::make_unique<assert_unit_tests::unit_tests_event_listener>(ss)).run();
  assert_value_("Start 1 test from 1 test case\n"
                "  SUCCEED test_assert_is_NaN_long_double_succeed.test_case_succeed\n"
