...
namespace tunit {
  ...
  class ostream_event_listener : public tunit::event_listener {
  public:
    ...
    void on_test_aborted(const tunit::test_event_args& e) const override {
      ...
      if (tunit::settings::default_settings().show_duration())
      ...
      if ((e.test().expect() != "" || e.test().actual() != "") && e.test().line_info() != tunit::line_info::empty()) {
    ...
    void on_test_failed(const tunit::test_event_args& e) const override {
      ...
      if (e.test().actual() != "")
        this->os_ << "    But was:  " << e.test().actual() << std::endl;
      if (e.test().line_info() != tunit::line_info::empty()) {
        this->os_ << "    Stack Trace: in " << e.test().line_info().file_path();
        if (e.test().line_info().line_number() != 0) this->os_ << ":" << e.test().line_info().line_number();
      }
      this->os_ << std::endl;
